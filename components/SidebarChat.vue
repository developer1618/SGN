  ...
  <div class="sidebar bg-[#1F2328] text-white flex h-full pr-12 pl-6 py-14 flex-col items-between justify-between">
    ...
    <div class="h-full">
      <div class="sidebar_user-info pb-16">
        <div>
        <nuxt-link to="/"><h3 class="text-base font-semibold">{{$auth.user.name }} {{$auth.user.lastname}}</h3></nuxt-link>
      </div>
        <div>
          <a :href="`tel:+992${$auth.user.phone}`" class="text-sm font-medium text-[#B3B9C9]">+992 {{$auth.user.phone}}</a>
        </div>
      </div>
      <div class="sidebar_navigation">
        <nav>
          <ul>
            <li class="pb-8 flex gap-5" :class="active === nav.title ? 'text-[#727ABE] active' : ''" @click="active = nav.title" v-for="nav in navigation"><nuxt-link class="flex gap-x-4" :to="nav.url">
              <svg >
    ...
    <div class="sidebar_exit">
      <div>
          <div class="flex cursor-pointer text-lg font-semibold" @click="onLogout" ><img :src="logout.icon" alt="icon" class="pr-4"><span>{{logout.title}}</span></div>
      </div>
    </div>
  </div>
</template>
