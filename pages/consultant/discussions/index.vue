    ...
    <Modal  @ondelete="ondelete(id)" />
      <AddModal v-if="isDiscussion" @onClose="isDiscussion = false"/>
        <div class="messages w-96 bg-white overflow-y-auto">
            ...
            <div class="absolute bottom-0 left-[28rem] pb-7" @click="isDiscussion = true">
        ...
        <div class="consultations w-full relative justify-center items-end pb-7 mt-20">
            <div class="messages px-10 absolute inset-x-0 bottom-0 mb-20 overflow-x-auto overscroll-y-none h-full">
                <div class="h-8 my-4 flex justify-between bg-white items-center py-6 px-5 rounded-lg">
                    <div v-for="(discussion, index) in discussion_name" :key="index">
...
export default{
    ...
    async getDiscussions(){
        ...
        // this.discussion_name = res.data.discussions;
        this.discussion_name.push(res.data.discussions[0])
    },
    async getSingleDiscussion(discussion){
        this.discussion_id = discussion.id;
        let res = await this.$axios.get(`/discussionPost/${discussion.id}`);
        this.discussionChat = res.data.discussionPosts;
        this.discussion_name = []
        this.discussion_name.push(discussion)
    },
    async onMessage(){
            if (!this.body.trim()) {
